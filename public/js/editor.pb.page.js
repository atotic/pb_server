...
"use strict";
		...
		generateDom: function(options) {
			...
			if ( options.editable && !options.enclosingDom) {
				$encloseDom.on( 'touchstart mousedown', function(ev) {
					PB.Page.Selection.findClosest($encloseDom)
						.setSelection(null, null, {updateClickTime: true });
					ev.stopPropagation();
				});
				PB.Page.Selection.bindToDom( this, $encloseDom )
			}
			if (options.editable) {
				PB.Page.Selection.findClosest($encloseDom).relayout();
			}
	...
	var PageProxyEditable = {
		...
		// 	var pageSelection = PB.Page.Selection.findClosest( $itemDom );
		...
		makeEditable: function(item, $itemDom) {
			...
			$itemDom.on('mousedown.select touchstart.select', function(ev) {
				THIS.selectItem( PB.Page.Selection.findClosest( $itemDom ),
		...
		makeItemSyncable: function(page, $itemDom, options) {
			$itemDom.on( PB.MODEL_CHANGED, function( ev, model, prop, eventOptions ) {
				...
				PB.Page.Selection.findClosest($pageDom).highlight();
				ev.stopPropagation();
			});
		},
		makePageSyncable: function($pageDom, options) {
			...
			$pageDom.on( PB.MODEL_CHANGED, function( ev, model, prop, eventOptions ) {
				switch( prop ) {
					...
					case 'assetList':
					case 'layoutId':
						var pageSelection = PB.Page.Selection.findClosest($pageDom);
